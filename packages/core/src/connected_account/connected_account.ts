...
export namespace ConnectedAccount {
    ...
    export const connectFacebook = fn(
        ...
        async (input) => {
            ...
            const { userInfo, pages } = await getFacebookUserData(tokenResponse.access_token);
    ...
    export const get = fn(z.object({ id: z.string() }), async ({ id }) => {
        ...
        return createTransaction(async (tx) => {
            const result = await tx
                ...
                .from(connectedAccount)
                .where(
                    and(eq(connectedAccount.id, id), eq(connectedAccount.workspaceID, workspaceID)),
                )
                .then((rows) => rows.at(0));
    ...
    export const getByPlatform = fn(z.object({ platform: Platform }), async ({ platform }) => {
        const workspaceID = Actor.workspaceID();
