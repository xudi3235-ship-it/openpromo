...
export type MetricsTimeSeriesInterval = "day" | "week";
...
export async function fetchWorkspaceMetricsTimeSeries(params: {
  workspaceId: string;
...
export async function fetchWorkspaceTopContent(params: {
  workspaceId: string;
  limit?: number;
  ...
  return rows
    .map((row) => {
      const metrics = ContentMetricsSummarySchema.safeParse(row.metrics ?? {});
