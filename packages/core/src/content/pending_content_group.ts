...
import { createTransaction, afterTx, Transaction } from "../drizzle/transaction";
...
export namespace PendingContentGroup {
    ...
    export const createWithUnifiedContents = fn(
        ...
        async (input) => {
            ...
            await createTransaction(async (tx) => {
                ...
                await createPendingContents(tx, input.placementSpecs, pending_content_group_id);
            });
    ...
    function serialize(input: typeof pendingContentGroupTable.$inferSelect): z.infer<typeof Info> {
        return {
            id: input.id,
