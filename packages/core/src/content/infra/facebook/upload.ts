...
export class FacebookUploader {
    ...
    private async startUploadSession(params: StartSessionParams): Promise<string> {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to start upload session: ${JSON.stringify(data)}`);
            }
    ...
    public async upload(filePath: string, mimeType: string): Promise<UploadResult> {
        ...
        const fileHandle = await this.transferFileData(uploadSessionId, fileData, 0);

        return { fileHandle, uploadSessionId };
    }

    public async resume(uploadSessionId: string, filePath: string): Promise<UploadResult> {
