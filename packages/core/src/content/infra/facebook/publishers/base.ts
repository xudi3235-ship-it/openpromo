import { AdVideo, FacebookAdsApi, Page, Photo } from "facebook-nodejs-business-sdk";
...
export abstract class BaseFacebookPublisher {
    ...
    // -------- low level apis -------
    protected async createFeed(fields: string[], params: CreateFeedParams): Promise<Page> {
        const { success, data: validatedParams, error } = CreateFeedSchema.safeParse(params);
        if (!success) throw new Error(`Invalid feed params: ${error.message}`);
        return await this.page.createFeed(fields, validatedParams);
    }

    protected async createPhoto(fields: string[], params: CreatePhotoParams): Promise<Photo> {
        const { success, data: validatedParams, error } = CreatePhotoSchema.safeParse(params);
     ...
     * @returns
     */
    protected async createVideo(fields: string[], params: CreateVideoParams): Promise<AdVideo> {
        const { success, data: validatedParams, error } = CreateVideoSchema.safeParse(params);
    ...
    public async createVideoReel(videoUrl: string, title: string, description: string) {
        const config = {
            pageId: this.page.id,
            accessToken: this.api.accessToken,
        };
        const { video_id, upload_url } = await FacebookReelProvider.startUploadSession(config);
