...
show_help() {
    ...
    echo "  2. Generate TypeScript Zod schemas via orval"
    echo "  3. Generate Internal API OpenAPI spec from ORPC"
    echo "  4. Generate Python SDK for Internal API"
}
...
if [ "$SKIP_MODAL" = false ]; then
    log_step "Step 1/4: Generate Python OpenAPI spec from Modal/FastAPI"
    cd "$ROOT_DIR/packages/backend"
    uv run modal run main.py::sdk
    log_success "Generated packages/backend/openapi.json"
else
    log_step "Step 1/4: Skipping Modal OpenAPI generation (--skip-modal)"
    log_success "Using existing packages/backend/openapi.json"
fi

# Step 2: Generate TypeScript Zod schemas via orval
log_step "Step 2/4: Generate TypeScript Zod schemas via orval"
cd "$ROOT_DIR/packages/scripts"
pnpm orval
log_success "Generated packages/shared/src/generated/openpromo_backend.zod.ts"

# Step 3: Generate Internal API OpenAPI spec from ORPC
log_step "Step 3/4: Generate Internal API OpenAPI spec from ORPC"
cd "$ROOT_DIR/packages/core"
pnpm gen:openapi
log_success "Generated packages/dash/worker/openapi-internal.json"

# Step 4: Generate Python SDK for Internal API
log_step "Step 4/4: Generate Python SDK for Internal API"
cd "$ROOT_DIR/packages/backend"
